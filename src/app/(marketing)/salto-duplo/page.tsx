...
import { Camera, Video, Zap, Sparkles, ArrowRight } from "lucide-react";
...
export default function SaltoDuploPage() {
    ...
    const GroupPromoSection = (
        <motion.div
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.7 }}
            className="mt-16 rounded-[40px] overflow-hidden bg-white border border-zinc-200 shadow-xl"
        >
            <div className="grid grid-cols-1 lg:grid-cols-2">
                ...
                <div className="p-10 lg:p-14 flex flex-col justify-center bg-zinc-50">
                    <span className="text-[#008800] font-black tracking-widest uppercase text-xs mb-3 bg-[#39FF14]/20 inline-block px-3 py-1 rounded-full w-fit">
                        Promoção Exclusiva
                    </span>
                    <h3 className="text-3xl md:text-4xl font-black italic uppercase tracking-tighter text-black mb-5">
                        Bora saltar com os amigos!
                    </h3>
                    <p className="text-zinc-600 font-medium leading-relaxed mb-6">
                        Venha em grupo e aproveite nossa oferta especial: traga <strong className="text-black">10 ou mais amigos</strong> e o seu salto sai completamente de graça! Se preferir dividir o benefício, todos do grupo ganham desconto especial na modalidade escolhida. <strong className="text-black">Você decide o que é melhor para o seu grupo!</strong>
                    </p>
                    <ul className="space-y-3 mb-8">
                        {[
                            "Traga 10+ amigos e salte de graça",
                            "Ou todos ganham desconto coletivo",
                            "Você escolhe o benefício que preferir",
                        ].map((item) => (
                            <li key={item} className="flex items-center gap-3 text-zinc-700 font-medium text-sm">
                                <span className="w-5 h-5 bg-[#39FF14] rounded-full flex items-center justify-center flex-shrink-0 text-black text-xs font-black">✓</span>
                                {item}
                            </li>
                        ))}
                    </ul>
                    <a
                        href={`https://wa.me/5515998282280?text=${encodeURIComponent("Olá! Tenho interesse na promoção de grupo para o Salto Duplo. Pode me dar mais informações?")}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-2 bg-black hover:bg-[#008800] text-white font-black italic uppercase px-8 py-4 rounded-2xl transition-all duration-300 hover:scale-105 w-fit text-sm"
                    >
                        Quero a promoção de grupo <ArrowRight className="w-4 h-4" />
                    </a>
                </div>
            </div>
        </motion.div>
    ...
    return (
        <>
            ...
            <ServicePageLayout
                ...
                renderBelowSteps={GroupPromoSection}
                steps={[
                    {
